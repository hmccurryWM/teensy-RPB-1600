...
bool RPB_1600::readWithCommand(uint8_t commandID, uint8_t receiveLength)
{
...
#ifdef RPB_1600_DEBUG
        Serial.print(F("<RPB-1600 DEBUG> Received byte "));
        Serial.print(num_bytes);
        Serial.print(F(" with value 0x"));
        Serial.println(my_rx_buffer[num_bytes], HEX);
        NL
#endif
    ...
    {
        Serial.print("0x");
        if (my_rx_buffer[i] < 0x10)  // Add leading zero if needed
        {
            Serial.print("0");
        }
        Serial.print(my_rx_buffer[i], HEX);
        if (i < num_bytes - 1)
        {
            Serial.print(",");
        }
...
bool RPB_1600::readManufData(mfr_data *mfrdata)
{
    ...
    for (int i = 0; i < sizeofthing; ++i)
    {
        //#ifdef RPB_1600_DEBUG
        Serial.print(F("Num: "));
        Serial.print(my_rx_buffer[i]);
        Serial.print("  Char: ");
            Serial.print((char)my_rx_buffer[i]);
            Serial.print(F("\n"));
        //#endif
        mfrdata->id[i] = my_rx_buffer[i];
    }
    ...
    for (int i = 0; i < sizeofthing; ++i)
    {
        mfrdata->model[i] = my_rx_buffer[i];
    }
    ...
    for (int i = 0; i < sizeofthing; ++i)
    {
        mfrdata->revision[i] = my_rx_buffer[i];
    }
    ...
    for (int i = 0; i < sizeofthing; ++i)
    {
        mfrdata->location[i] = my_rx_buffer[i];
    }
    ...
    for (int i = 0; i < sizeofthing; ++i)
    {
        mfrdata->serial[i] = my_rx_buffer[i];
    }
}
